...
# ## Proof of Concept dengan Google Colab
#
# Fitur:
...
# %% [markdown]
# ## 🔍 Virtual Quality Control with Computer Vision Simulation
    ...
    def visualize_predictive_maintenance(self, sensor_data, anomalies):
        ...
        if len(anomaly_dates) > 0:
            ...
            if time_to_failure > 24:
                print("⚠️  CRITICAL: Immediate maintenance required!")
...
def create_interactive_interface():
    ...
    with gr.Blocks(title="AI CNC Learning Platform") as demo:
        gr.Markdown("# 🏭 AI CNC Virtual Learning Platform")
...
Untuk menjalankan proyek ini di lingkungan Google Colab, Anda perlu menginstal *library* Python yang diperlukan. Salin dan jalankan sel berikut di notebook Colab Anda:

```python
# Install required libraries
!pip install numpy matplotlib plotly ipywidgets scikit-learn tensorflow gradio -q
!apt-get install -y xvfb libgtk2.0-0 libgconf-2-4 libxss1 libnss3 libasound2 > /dev/null 2>&1
```
...
*   Salin seluruh kode dari notebook ini (mulai dari instalasi library hingga fungsi `test_all_features()` dan Gradio/Streamlit) ke dalam notebook Colab Anda.
*   Jalankan setiap sel kode secara berurutan. Pastikan sel instalasi (`!pip install...`) dijalankan pertama kali.
...
*   **Visualisasi**: Visualisasi 2D (menggunakan Matplotlib) dan 3D (menggunakan Plotly) akan ditampilkan secara langsung di output sel Colab. Jika dijalankan secara lokal, window Matplotlib akan muncul, dan plot Plotly akan terbuka di browser default Anda.
...
*   **Visualisasi**: Gambar hasil inspeksi (benda kerja asli, deteksi tepi, dan cacat yang disorot) akan ditampilkan langsung di output sel Colab menggunakan Matplotlib. Jika dijalankan secara lokal, ini akan muncul di window Matplotlib.
...
*   **Visualisasi**: Grafik interaktif dari data sensor dengan anomali yang disorot (menggunakan Plotly) akan ditampilkan di output sel Colab. Jika dijalankan secara lokal, plot ini akan terbuka di browser default Anda.
...
# # %% [markdown]
# # ## 📱 Streamlit App for Mobile Access
# #
...
# 
# # Title
# st.title("🏭 AI CNC Virtual Learning Platform - Mobile Edition")
...
def test_all_features():
    ...
    simulator = SimpleCNCSimulator()
    path = simulator.execute_gcode(sample_gcode)
    print(f"✓ CNC Simulator: {len(path)} points generated")

    # Test AI Assistant
    ai = AICodeAssistant()
    code = ai.generate_square()
    print(f"✓ AI Assistant: G-code generated ({len(code.split())} lines)")

    # Test Quality Control
    qc = VirtualQualityControl()
    workpiece = qc.simulate_workpiece()
    defects = qc.detect_defects(workpiece)
    print(f"✓ Quality Control: {len(defects)} defects detected")

    # Test Predictive Maintenance
    pm = PredictiveMaintenanceSimulator()
    data = pm.generate_sensor_data(days=10)
    anomalies = pm.detect_anomalies(data)
    print(f"✓ Predictive Maintenance: {sum(anomalies)} anomalies found")

    print("\n✅ All prototype features working correctly!")
    return True
...
*   Kelas `SimpleCNCSimulator` memungkinkan Anda mengeksekusi G-code dan memvisualisasikan jalur alat.
*   Anda dapat memodifikasi `sample_gcode` di sel `VdaOTioDTz2n` dan menjalankan ulang untuk melihat visualisasi yang berbeda.
*   Visualisasi 2D dan 3D akan ditampilkan secara langsung di output sel.
...
### 5. Fitur Predictive Maintenance Simulation
*   Kelas `PredictiveMaintenanceSimulator` di sel `xz27_LKITzta` (dan versi yang sudah dimodifikasi untuk *grid search* di sel `07283e14`) mensimulasikan data sensor dan mendeteksi anomali.
*   Fungsi `pm.generate_sensor_data()` membuat data sensor, dan `pm.detect_anomalies()` mengidentifikasi pola-pola aneh.
*   `pm.visualize_predictive_maintenance()` akan menampilkan grafik sensor dengan anomali yang disorot serta prediksi pemeliharaan.
...
*   Sel `DHtHAInqTzqT` akan meluncurkan antarmuka web Gradio.
*   Setelah sel ini dijalankan, akan muncul tautan publik (biasanya berakhir dengan `.gradio.live`). Buka tautan ini di browser Anda untuk berinteraksi dengan semua fitur proyek dalam satu antarmuka yang ramah pengguna.
*   **Penting**: Antarmuka Gradio akan berjalan selama sel ini aktif. Jika sel dihentikan, antarmuka juga akan berhenti.
...
### 8. Pengujian Fungsionalitas Dasar
*   Sel `30vsjAIBTzkn` berisi fungsi `test_all_features()` yang dapat Anda jalankan untuk memverifikasi bahwa semua komponen utama proyek berfungsi dengan benar.
...
"""# Task
Implement hyperparameter grid search for the `OneClassSVM` model in the `detect_anomalies` method within the `PredictiveMaintenanceSimulator` class, varying `nu` and `gamma` parameters. Select the best model by choosing hyperparameters that result in an anomaly count closest to 5% of the total data points. Visualize the anomaly detection results using this best model and summarize the findings from this hyperparameter tuning process.

## Implement Hyperparameter Grid Search

### Subtask:
Modify the `detect_anomalies` method in the `PredictiveMaintenanceSimulator` class to perform a grid search for `OneClassSVM` hyperparameters (`nu` and `gamma`) and select the best combination based on a target anomaly percentage.

**Reasoning**:
The subtask requires modifying the `detect_anomalies` method within the `PredictiveMaintenanceSimulator` class to include a hyperparameter grid search for `OneClassSVM`. This involves redefining the class with the updated method, as specified in the instructions, to find the optimal `nu` and `gamma` values based on a target anomaly percentage. The previous `PredictiveMaintenanceSimulator` class and its `detect_anomalies` method are in cell `xz27_LKITzta`, so I will recreate the class and method with the grid search logic.
"""
...
class PredictiveMaintenanceSimulator:
    ...
    def detect_anomalies(self, sensor_data):
        """Detect anomalies using machine learning with hyperparameter grid search"""
        # Prepare data
        X = sensor_data.values
        ...
        min_anomaly_diff = float('inf')

        # Set target anomaly percentage and count
        target_anomaly_percentage = 0.05
        total_data_points = len(X_scaled)
        target_anomaly_count = target_anomaly_percentage * total_data_points

        print(f"Target anomaly count: {target_anomaly_count:.2f}")

        # Perform grid search
        for nu in nu_values:
            for gamma in gamma_values:
                # Create OneClassSVM model instance
                ...
                # Calculate current anomaly count
                current_anomaly_count = np.sum(predictions == -1)

                # Calculate anomaly difference from target
                anomaly_diff = abs(current_anomaly_count - target_anomaly_count)

                # Update best parameters if current model is better
                if anomaly_diff < min_anomaly_diff:
                    min_anomaly_diff = anomaly_diff
                    best_nu = nu
                    best_gamma = gamma
                    best_predictions = predictions

        # Update anomalies variable with the best model's predictions
        anomalies = best_predictions == -1

        # Print chosen best parameters and total anomalies found
        print(f"Best OneClassSVM parameters: nu={best_nu}, gamma={best_gamma}")
        print(f"Anomalies with best model: {np.sum(best_predictions == -1)}")
...
### Subtask:
Summarize the findings from applying cross-validation (hyperparameter tuning) to the OneClassSVM model, highlighting how the best parameters were chosen and the resulting anomaly detection.

## Summary:

### Q&A
The findings from applying hyperparameter tuning to the OneClassSVM model are as follows: the best parameters were chosen by conducting a grid search across specified `nu` and `gamma` values, aiming for an anomaly count closest to 5% of the total data points. This process successfully identified `nu=0.05` and `gamma='scale'` as the optimal parameters, resulting in an anomaly detection rate of 5.1%, which closely matched the target.

### Data Analysis Key Findings
*   The `detect_anomalies` method was enhanced to perform a grid search for `OneClassSVM` hyperparameters, specifically for `nu` (values: 0.01, 0.05, 0.1, 0.2) and `gamma` (values: 'scale', 'auto', 0.01, 0.1, 1).
*   The selection criterion for the best model was to identify hyperparameters that yielded an anomaly count closest to a target of 5% of the total data points.
*   For the dataset of 720 data points, the target anomaly count was 36.00.
*   The grid search successfully identified `nu=0.05` and `gamma='scale'` as the best OneClassSVM parameters.
*   Using these optimal parameters, the model detected 37 anomalies, which translated to an anomaly rate of 5.1%, closely aligning with the 5% target.
*   The visualization of the results, incorporating the best model, indicated that anomalies were detected early, leading to a critical recommendation for immediate maintenance.

### Insights or Next Steps
*   The hyperparameter tuning strategy effectively identified a OneClassSVM model configuration (`nu=0.05`, `gamma='scale'`) that closely aligns with a desired anomaly percentage, demonstrating a practical approach to controlling the sensitivity of anomaly detection.
*   For future enhancements, explore incorporating more sophisticated cross-validation techniques (e.g., K-fold cross-validation) to ensure the robustness of the chosen hyperparameters and potentially refine the target anomaly percentage based on domain expertise or historical failure data.
"""